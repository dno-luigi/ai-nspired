...
<head>
  ...
  <style>
    ...
    /* Card Flip */
    .card-flip-container { perspective: 1000px; width: 100%; min-height: 200px; }
    .card-flip-inner { position: relative; width: 100%; transition: transform 0.5s; transform-style: preserve-3d; }
    .card-flip-inner.flipped { transform: rotateY(180deg); }
    .card-flip-front, .card-flip-back { backface-visibility: hidden; -webkit-backface-visibility: hidden; width: 100%; }
    .card-flip-back { position: absolute; top: 0; left: 0; transform: rotateY(180deg); min-height: 200px; }
    .flip-btn { padding: 4px 10px; background: #2a2a4a; border: 1px solid #4a4a6a; border-radius: 4px; color: #e0e0e0; font-size: 10px; cursor: pointer; transition: all 0.15s; }
    .flip-btn:hover { border-color: #ff6b1a; color: #ff6b1a; }
    .elevate-btn { padding: 6px 14px; background: #1a4a2a; border: 1px solid #2a6a3a; border-radius: 6px; color: #4aff8a; font-size: 11px; font-weight: 600; cursor: pointer; transition: all 0.15s; }
    .elevate-btn:hover { background: #2a6a3a; border-color: #4aff8a; }
    /* Card Picker Modal */
  ...
  <script>
    ...
    function openTabList(filterType) {
      ...
      if (filtered.length === 0) {
        createCard({ type: "Document", title: icon + " " + typeLabel, content: "<div style=\"color:#666;font-size:12px;padding:16px;text-align:center;\">No " + typeLabel + " found. Run a " + (filterType === "eitl" ? "review" : filterType.toLowerCase()) + " to populate this list.</div>", rawHtml: true, domains: ["general"] });
        return;
      }
      const sorted = filtered.sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""));
      const listHtml = "<div class=\"picker-list\">" + sorted.map(c => {
        ...
        const eitlBadge = c.eitlStatus ? "<span style=\"color:" + (c.eitlStatus === "elevated" ? "#4aff8a" : c.eitlStatus === "remanded" ? "#ff4a4a" : "#ffaa4a") + ";font-size:9px;margin-left:4px;\">" + c.eitlStatus.toUpperCase() + "</span>" : "";
        return "<div class=\"picker-item\" data-card-id=\"" + c.id + "\">"
          + "<span class=\"picker-badge\" style=\"background:" + badge + "22;color:" + badge + ";\">" + c.type + "</span>"
          + "<span class=\"picker-title\">" + (c.title || "Untitled") + eitlBadge + "</span>"
      ...
      const cardHtml = "<div style=\"padding:12px;\">"
        + "<div style=\"display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;\">"
        + "<span style=\"font-size:11px;color:#888;\">" + filtered.length + " items — click to open, edit to flip, elevate to save to knowledge</span>"
        + "<button onclick=\"syncFromKV()\" class=\"ctrl-btn\" style=\"font-size:10px;padding:2px 8px;\">Sync</button>"
        + "</div>" + listHtml + "</div>";
      createCard({ type: "Document", title: icon + " " + typeLabel, content: cardHtml, rawHtml: true, splitView: false, domains: ["general"] });
    }
    ...
    function flipCardEdit(cardId) {
      ...
      if (el) {
        ...
        if (contentArea) contentArea.innerHTML = flipHtml;
      }
    }
    ...
    async function elevateCardContent(cardId) {
      ...
      const title = card?.title || "Elevated knowledge";
      showToast("Elevating knowledge...", "processing");
      try {
        const res = await fetch("/api/elevate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ cardId, revisedContent: content, title, confidence: 0.7 })
        });
        const data = await res.json();
        if (data.ok) {
          showToast("Knowledge elevated to KV", "success");
          card.content = content;
          card.eitlStatus = "elevated";
          updateCardDOM(card);
        } else {
          showToast("Elevate failed: " + (data.error || "unknown"), "error");
        }
      } catch (e) {
        showToast("Elevate error: " + e.message, "error");
      }
    }
    ...
    async function elevateFromPicker(cardId) {
      ...
      try {
        const res = await fetch("/api/elevate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ cardId, revisedContent: card.content, title: card.title, confidence: 0.7 })
        });
        const data = await res.json();
        if (data.ok) {
          showToast("Knowledge elevated", "success");
          card.eitlStatus = "elevated";
          updateCardDOM(card);
        } else {
          showToast("Elevate failed: " + (data.error || "unknown"), "error");
